...
const clickToNextMapping = () => {
    ...
    if(importExport.booking.import_column.length === 1 && importExport.booking.import_column[0] === ""){
        toast.error(tfhb_core_apps.trans[(tfhb_core_apps.trans['Selected  File column is empty!'] || 'Selected  File column is empty!')], {
            position: 'bottom-right', // Set the desired position
            autoClose: 1500,
        });
        return false;
 ...
 <div class="tfhb-import-cotent tfhb-flexbox tfhb-justify-center" :class="{ 'tfhb-skeleton': skeleton }">
    ...
    <div v-if="importExport.booking.steps =='start'" class="tfhb-import-wrap tfhb-flexbox tfhb-gap-24">     
        ...
        <div class="tfhb-admin-title" > 
            ...
            <p>{{ $tfhb_trans('Need a sample template?') }} <span  @click.stop="importExport.ExportSampleData('bookings')">{{ $tfhb_trans('Download one') }} </span> {{ $tfhb_trans('here.') }}</p>
        </div>
        <HbFileUpload
            name="dashboard_logo"
