...
async function fetchUserPublicInfo() {
    loading.value = true;
    try {
        const response = await axios.get(tfhb_core_apps.rest_route + 'hydra-booking/v1/buyers/user-public-info', {
            headers: {
...
async function changePassword() {
    ...
    try {
        const response = await axios.post(
            tfhb_core_apps.rest_route + 'hydra-booking/v1/buyers/change-password',
            { ...passwordForm },
            {
                headers: {
                    'X-WP-Nonce': tfhb_core_apps.rest_nonce,
...
<template>   
    ...
    <div class="tfhb-admin-card-box tfhb-flexbox tfhb-mb-24">  
        ...
        <HbTextarea  
            ...
            width="100"  
        />  
        <!-- Add more fields as needed for preferredWorkshops, regionsOfInterest, etc. -->
         <HbButton 
